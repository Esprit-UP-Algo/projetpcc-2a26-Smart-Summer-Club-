...
String longText = "";
int displayMode = 0; // 0: Main view (date + activity)
unsigned long lastModeChange = 0;
const unsigned long MODE_DURATION = 999999; // Effectively disabled - display stays on main view
...
void updateActivityDisplay() {
  // Display mode is fixed to 0 (main view) to allow Qt app to control activity switching
  // This prevents Arduino from cycling through different views which would conflict
  // with the Qt application's activity cycling timer
  
  displayMode = 0; // Always show main view
  displayDateAndActivityType();
}
...
// Simplified display showing date on line 1 and activity type on line 2
void displayDateAndActivityType() {
  lcd.clear();
  ...
  // Line 2: Activity Type (truncated if too long)
  lcd.setCursor(0, 1);
  String activityType = currentActivity.type;
  if (activityType.length() > 16) {
    activityType = activityType.substring(0, 16);
  }
