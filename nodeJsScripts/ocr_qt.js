// ocr_qt.js - Enhanced OCR with Arabic to English translation and JSON output
import fs from 'fs';
...
// Quiet mode for Qt integration (suppress debug output)
const QUIET_MODE = true;
const log = QUIET_MODE ? () => {} : console.log;
const logError = QUIET_MODE ? () => {} : console.error;
...
// Usage: node ocr_qt.js ./image.png (Qt Integration Version - JSON Output Only)
const inputImagePath = process.argv[2];
if (!inputImagePath) {
  process.stderr.write('Usage: node ocr_qt.js <image_path>\n');
  process.exit(1);
}

imageToText(inputImagePath)
  .then((result) => {
    // Output ONLY the structured JSON data for Qt to parse
    const outputData = {
  ...
  .catch((error) => {
    // Output error as JSON for Qt to handle
    const errorData = {
      error: true,
